...
if ($method === 'POST') {
    ...
    if ($action === 'push_to_site') {
        ...
        // 音质解析接口
        $apiStandard = "https://music.rczx.asia/song/url/v1?id=" . $musicId . "&level=standard";
        $apiHigh = "https://music.rczx.asia/song/url/v1?id=" . $musicId . "&level=lossless";
...
if ($method === 'GET') {
    ...
    if ($action === 'music_url') {
        ...
        // 默认音质为 standard
        $target = "https://music.rczx.asia/song/url/v1?id={$id}&level=standard";
        $url = fetchUrl($target);
        ob_clean();
